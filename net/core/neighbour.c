...
static void neigh_hash_free_rcu(struct rcu_head *head)
{
	struct neigh_hash_table *nht = container_of(head,
						    ...
						    rcu);
	size_t size = (nht->hash_shift + 1) * sizeof(struct neighbour *);
						...
						unsigned long new_entries)
{
	...
	for (i = 0; i <= old_nht->hash_shift; i++) {
				     ...
				     const void *pkey)
{
	...
	rcu_read_lock_bh();
	nht = rcu_dereference_bh(tbl->nht);
	hash_val = tbl->hash(pkey, NULL, nht->hash_rnd) & nht->hash_shift;
			       ...
			       struct net_device *dev)
{
	...
	if (atomic_read(&tbl->entries) > (nht->hash_shift + 1))
		nht = neigh_hash_grow(tbl, (nht->hash_shift + 1) << 1);
			      ...
			      u32 pid, u32 seq, int type, int flags)
{
...

	{
		...
		ndc.ndtc_hash_mask = nht->hash_shift;
		rcu_read_unlock_bh();
...
void neigh_for_each(struct neigh_table *tbl, void (*cb)(struct neighbour *, void *), void *cookie)
{
	...
	for (chain = 0; chain <= nht->hash_shift; chain++) {
		struct neighbour *n;
...
static struct neighbour *neigh_get_first(struct seq_file *seq)
{
	...
	for (bucket = 0; bucket <= nht->hash_shift; bucket++) {
...
next:
		...
		if (++state->bucket > nht->hash_shift)
			break;
