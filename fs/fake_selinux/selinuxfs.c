...
unsigned int selinux_checkreqprot = 1;
...
static unsigned int global_handle_unknown = 0;
					...
					size_t count, loff_t *ppos)
{
	...
	int handle_unknown = /*(ino == SEL_REJECT_UNKNOWN) ?
		security_get_reject_unknown() : !security_get_allow_unknown()*/ global_handle_unknown;
...
static int sel_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	pr_err("%s: simple_fill_super\n", __func__);
	ret = simple_fill_super(sb, SELINUX_MAGIC, selinux_files);
	if (ret)
		goto err;

	pr_err("%s: sel_make_dir(sb->s_root, BOOL_DIR_NAME, &sel_last_ino);\n", __func__);
	...
	pr_err("%s: d_alloc_name(sb->s_root, NULL_FILE_NAME);\n", __func__);
	dentry = d_alloc_name(sb->s_root, NULL_FILE_NAME);
	if (!dentry)
		goto err;

	ret = -ENOMEM;
	pr_err("%s: sel_make_inode(sb, S_IFCHR | S_IRUGO | S_IWUGO);\n", __func__);
...
MODULE_AUTHOR("Shilin Victor aka ChronoMonochrome <chrono.monochrome@gmail.com>");
MODULE_DESCRIPTION("Fake SELinux implementation");
MODULE_LICENSE("GPL");

module_init(init_sel_fs);
module_exit(exit_sel_fs);
